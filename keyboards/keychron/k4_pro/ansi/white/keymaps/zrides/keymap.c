...
static bool m_fn_keys_active = false;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [WIN_BASE] = LAYOUT_ansi_100(
        KC_ESC,   KC_BRID,  KC_BRIU,  KC_TASK,  KC_FILE,  BL_DOWN,  BL_UP,    KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,    KC_VOLU,  KC_DEL,   KC_HOME,  KC_END,   KC_PGUP,  KC_PGDN,  BL_STEP,
        KC_GRV,   KC_1,     KC_2,     KC_3,     KC_4,     KC_5,     KC_6,     KC_7,     KC_8,     KC_9,     KC_0,     KC_MINS,    KC_EQL,   KC_BSPC,            KC_NUM,   KC_PSLS,  KC_PAST,  KC_PMNS,
        KC_TAB,   KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,     KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,     KC_LBRC,    KC_RBRC,  KC_BSLS,            KC_P7,    KC_P8,    KC_P9,    KC_PPLS,
        KC_CAPS,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,     KC_H,     KC_J,     KC_K,     KC_L,     KC_SCLN,  KC_QUOT,              KC_ENT,             KC_P4,    KC_P5,    KC_P6,
        KC_LSFT,            KC_Z,     KC_X,     KC_C,     KC_V,     KC_B,     KC_N,     KC_M,     KC_COMM,  KC_DOT,   KC_SLSH,              KC_RSFT,  KC_UP,    KC_P1,    KC_P2,    KC_P3,    KC_PENT,
        KC_LCTL,  KC_LWIN,  KC_LALT,                                KC_SPC,                                 KC_RALT,  TT(WIN_FN), KC_RCTL,  KC_LEFT,  KC_DOWN,  KC_RGHT,  KC_P0,    KC_PDOT          ),
...
// uint16_t getKeyFromIndex(uint8_t layer, uint8_t index) {
...
// We need to account for the fact that the LED matrix has some NO_LED values so lengths are not equal
uint16_t getKeyFromLEDConfig(uint8_t layer, uint8_t led_index) {
    ...
    return KC_NO;  // Return a default value if not found
}

bool led_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max){
    if(m_fn_keys_active){
        for (uint8_t i = led_min; i < led_max; i++) {
            // get current led index as the keycode
            uint16_t currentKeycode = getKeyFromLEDConfig(WIN_FN, i);
            if (currentKeycode == KC_TRNS) {
                led_matrix_set_value(i, 0x0);
            }
            else {
